	...
	JY=JY.prototype={
		...
		getCtx:function(id,doc){
			if (! this.byId(id,doc).getContext){
				alert('您的浏览器不支持html元素，请更换浏览器，推荐使用360极速或谷歌或ie9以上版本的浏览器内核.');
			};
		...
		loadFile:function (src,type,callback,flashId){  
			...
			if (type ==="script"){
				...
				JY.each(arr,function(v,i){
					...
					JY.css(div,{width:'0px',height:'0px',"overflow":'hidden'});
					JY.append(div,elem);	 
		...
		progressbar:function(cur,max,maxWidth){
			...
			JY.css(percent,{'position':"absolute","textAlign":"center","top":'0px',width:maxWidth+"px",'text-align':"center"});
	...
	JY.draw={
		...
		line:function(objA,objB,c,callback){
			...
			JY.resolve(function(){				
				...
				objA.x=x;
				objA.y=y;
				callback ?callback({x:x,y:y}):null;
			},1,len);
		}
	};
	win.JY=JY;
...
(function(){
	...
	function Sprite(w,h,style){
		...
		this.init = function(w,h,style){
			...
			JY.extend(this.style,{position:"absolute",overflow:"hidden",width:this.width +"px",height:this.height+"px"});
